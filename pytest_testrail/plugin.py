...
class PyTestRailPlugin(object):
    def __init__(self, client, assign_user_id, project_id, suite_id, include_all, cert_check, tr_name, tr_description='', run_id=0,
                 plan_id=0, version='', close_on_complete=False, publish_blocked=True, skip_missing=False, retest_failed=False, retest_all=False,
    ...
    def pytest_collection_modifyitems(self, session, config, items):
        ...
        if self.testplan_id and self.is_testplan_available():
            ...
            if self.skip_missing:
                status_ids = [TESTRAIL_TEST_STATUS["untested"], TESTRAIL_TEST_STATUS["retest"]]
                if self.retest_all or self.retest_failed:
                    status_ids.append(TESTRAIL_TEST_STATUS["failed"])
                if self.retest_all:
                    status_ids.append(TESTRAIL_TEST_STATUS["passed"])
#                print("status ids:", status_ids)
                tests = self.get_tests(self.testrun_id)
                tests_list = [
                    test.get('case_id') for test in tests if test.get('status_id') in status_ids
                ]
#                print("tests list", tests_list)
                for item, case_id in items_with_tr_keys:
                    if not set(case_id).intersection(set(tests_list)):
                        mark = pytest.mark.skip('Test case is excluded in testrun.')
        ...
        if item.get_closest_marker(TESTRAIL_PREFIX):
            ...
            if rep.when == 'call' and testcaseids:
                if rep.longrepr is not None:
                    #mycomment = str(rep.longrepr) + item.module.cap_text
                    mycomment = str(rep.longrepr) + "\n" + pytest.cap_text

                else:
                    #mycomment = item.module.cap_text
                    mycomment = pytest.cap_text
